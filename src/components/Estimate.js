...
export default function Estimate() {
  ...
  const nextQuestion = () => {
    ...
    const activeIdx = activeQuestion.id - 1;
    const nextIdx = activeIdx + 1;

    newQuestions[activeIdx] = { ...activeQuestion, active: false };
    newQuestions[nextIdx] = { ...newQuestions[nextIdx], active: true };
  ...
  const navigationPrevDisabled = () => {
    const currentActive = questions.filter((question) => question.active)[0];
    console.log(currentActive);
    if (currentActive.id === 1) return true;
    return false;
  };

  const navigationNextDisabled = () => {
    const currentActive = questions.filter((question) => question.active)[0];
    if (currentActive.id === questions.length) return true;
    return false;
  };
  ...
  const handleSelect = (id) => {
    ...
    const newSelected = newQuestions[activeIdx].options[id - 1];
    const prevSelected = activeQuestion.options.filter(
      (option) => option.selected
    )[0];

    switch (activeQuestion.subtitle) {
      case "Select one.":
        if (prevSelected) {
